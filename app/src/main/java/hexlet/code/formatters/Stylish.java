    ...
    public static String format(Map<String, Map<String, Object>> data, List<String> keys) {
        ...
        for (String property: keys) {
            if (data.get(property).containsKey("removed") && !data.get(property).containsKey("added")) {
                result.add(" ".repeat(2) + "- " + property + ": " + data.get(property).get("removed"));
            }
            if (!data.get(property).containsKey("removed") && data.get(property).containsKey("added")) {
                result.add(" ".repeat(2) + "+ " + property + ": " + data.get(property).get("added"));
            }
            if (data.get(property).containsKey("unchanged")) {
                result.add(" ".repeat(4) + property + ": " + data.get(property).get("unchanged"));
            }
            if (data.get(property).containsKey("removed") && data.get(property).containsKey("added")) {
                result.add(" ".repeat(2) + "- " + property + ": " + data.get(property).get("removed"));
                result.add(" ".repeat(2) + "+ " + property + ": " + data.get(property).get("added"));
